...
resource "aws_autoscaling_group" "master" {
  ...
  target_group_arns = [
    ...
    "${aws_alb_target_group.master_http_8181.arn}",
    "${aws_alb_target_group.master_http_2181.arn}"
  ]
...
/* HTTP 8181 */
resource "aws_alb_target_group" "master_http_8181" {
  name     = "${var.pre_tag}-Master-8181"
  port     = 8181
  protocol = "HTTP"
  vpc_id   = "${var.vpc_id}"
...
resource "aws_alb_listener" "master_http_8181" {
  load_balancer_arn = "${aws_alb.master.arn}"
  port = "8181"
  protocol = "HTTP"

  default_action {
    target_group_arn = "${aws_alb_target_group.master_http_8181.arn}"
    type = "forward"
  }
}

/* HTTP 2181 */
resource "aws_alb_target_group" "master_http_2181" {
  name     = "${var.pre_tag}-Master-2181"
  port     = 2181
  protocol = "HTTP"
  vpc_id   = "${var.vpc_id}"
...
resource "aws_alb_listener" "master_http_2181" {
  load_balancer_arn = "${aws_alb.master.arn}"
  port = "2181"
  protocol = "HTTP"

  default_action {
    target_group_arn = "${aws_alb_target_group.master_http_2181.arn}"
    type = "forward"
  }
}
/* Route 53 */
